--[[ BLOXBOX UI v2.3 PREMIUM | by Samir & Team | Single-File | Material Icons ]]
...
local function Grad(parent,c1,c2,rot)local g=Instance.new("UIGradient");g.Color=ColorSequence.new(c1,c2);g.Rotation=rot or 135;g.Parent=parent;return g end
local function Divider(parent,pos)local d=Instance.new("Frame");d.Size=UDim2.new(0,1,0,18);d.Position=pos;d.BackgroundColor3=Color3.fromRGB(55,55,72);d.BackgroundTransparency=0.3;d.BorderSizePixel=0;d.Parent=parent;Corner(d,1);return d end

-- Executor Detection
local function DetectExecutor()
	local n="Unknown"
	pcall(function()if identifyexecutor then n=identifyexecutor()elseif getexecutorname then n=getexecutorname()elseif syn then n="Synapse X"elseif fluxus then n="Fluxus"elseif KRNL_LOADED then n="KRNL"end end)
	return n
end
...
local T={
	...
	AccSoft=Color3.fromRGB(88,128,255),Text=Color3.fromRGB(235,235,242),TextDim=Color3.fromRGB(130,130,155),
	TextMut=Color3.fromRGB(75,75,100),Border=Color3.fromRGB(42,42,58),Succ=Color3.fromRGB(75,200,115),
	Warn=Color3.fromRGB(255,185,55),Err=Color3.fromRGB(235,65,65),
	Font=Enum.Font.GothamMedium,FontB=Enum.Font.GothamBold,FontL=Enum.Font.Gotham,
}
...
local function Notif(o)
	...
	local iNames={Success="check_circle",Error="error",Warning="warning",Info="info"}
	ic.Image=Icon(iNames[o.Type or"Info"]or"info")
...
local function ShowIntro(logoId,cb)
	...
	Grad(card,Color3.fromRGB(30,30,42),Color3.fromRGB(20,20,28))
	...
	local sub=Instance.new("TextLabel");sub.Size=UDim2.new(1,0,0,14);sub.Position=UDim2.new(0.5,0,0,94);sub.AnchorPoint=Vector2.new(0.5,0);sub.BackgroundTransparency=1;sub.Text="v2.3 Premium";sub.TextColor3=T.TextMut;sub.Font=T.FontL;sub.TextSize=10;sub.TextTransparency=1;sub.Parent=card
...
local function CButton(tab,o)
	...
	local ic=Instance.new("ImageLabel");ic.Size=UDim2.fromOffset(16,16);ic.Position=UDim2.fromOffset(10,9);ic.BackgroundTransparency=1;ic.ImageColor3=T.Accent;ic.Image=Icon("play_arrow");ic.Parent=b
...
local function CDropdown(tab,o)
	...
	local arIc=Instance.new("ImageLabel");arIc.Size=UDim2.fromOffset(14,14);arIc.Position=UDim2.new(1,-20,0.5,0);arIc.AnchorPoint=Vector2.new(0,0.5);arIc.BackgroundTransparency=1;arIc.ImageColor3=T.TextMut;arIc.Image=Icon("expand_more");arIc.Parent=hd
	local sl=Instance.new("TextLabel");sl.Size=UDim2.fromOffset(55,18);sl.Position=UDim2.new(1,-76,0.5,0);sl.AnchorPoint=Vector2.new(0,0.5);sl.BackgroundTransparency=1;sl.Text=o.Default or"...";sl.TextColor3=T.Accent;sl.Font=T.Font;sl.TextSize=11;sl.TextXAlignment=Enum.TextXAlignment.Right;sl.Parent=hd
	...
	for _,item in ipairs(o.List or{})do
		...
		b.MouseButton1Click:Connect(function()sl.Text=item;st:Set(o.Flag,item);if o.Callback then o.Callback(item)end;op=false;Tw(c,{Size=UDim2.new(1,0,0,34)},"Fast");arIc.Image=Icon("expand_more")end)
	end
	hd.MouseButton1Click:Connect(function()op=not op;arIc.Image=Icon(op and"expand_less"or"expand_more");Tw(c,{Size=UDim2.new(1,0,0,op and(36+#(o.List or{})*28+4)or 34)},"Fast")end)
...
-- Tab (REDESIGNED sidebar tabs with icon + active indicator)
local TabIcons={"home","settings","info","build","star","explore","extension","dashboard"}
local Tab={};Tab.__index=Tab
function Tab.new(w,name,idx)local self=setmetatable({_w=w,_lib=w._lib,_name=name,_idx=idx or 1},Tab);self:_build();return self end
function Tab:_build()
	local tb=Instance.new("TextButton");tb.Size=UDim2.new(1,0,0,34);tb.BackgroundTransparency=1;tb.BackgroundColor3=T.SurfHov;tb.BorderSizePixel=0;tb.Text="";tb.AutoButtonColor=false;tb.Parent=self._w._tl;Corner(tb,6)
	self._tb=tb

	-- Active indicator bar (left side accent line)
	local ind=Instance.new("Frame");ind.Size=UDim2.new(0,3,0,18);ind.Position=UDim2.fromOffset(0,8);ind.BackgroundColor3=T.Accent;ind.BackgroundTransparency=1;ind.BorderSizePixel=0;ind.Parent=tb;Corner(ind,2)
	self._ind=ind

	-- Tab icon (Material)
	local icName=TabIcons[self._idx]or"circle"
	local ic=Instance.new("ImageLabel");ic.Size=UDim2.fromOffset(16,16);ic.Position=UDim2.fromOffset(12,9);ic.BackgroundTransparency=1;ic.ImageColor3=T.TextMut;ic.Image=Icon(icName);ic.Parent=tb
	self._ic=ic

	-- Tab name
	local nm=Instance.new("TextLabel");nm.Size=UDim2.new(1,-36,1,0);nm.Position=UDim2.fromOffset(34,0);nm.BackgroundTransparency=1;nm.Text=self._name;nm.TextColor3=T.TextDim;nm.Font=T.Font;nm.TextSize=12;nm.TextXAlignment=Enum.TextXAlignment.Left;nm.Parent=tb
	self._nm=nm
	...
	tb.MouseEnter:Connect(function()if self._w._sel~=self then Tw(tb,{BackgroundTransparency=0.8},"Fast");Tw(ic,{ImageColor3=T.TextDim},"Fast")end end)
	tb.MouseLeave:Connect(function()if self._w._sel~=self then Tw(tb,{BackgroundTransparency=1},"Fast");Tw(ic,{ImageColor3=T.TextMut},"Fast")end end)
end
function Tab:Show()
	self._ct.Visible=true
	Tw(self._tb,{BackgroundTransparency=0.75},"Fast")
	Tw(self._nm,{TextColor3=T.Accent},"Fast")
	Tw(self._ic,{ImageColor3=T.Accent},"Fast")
	Tw(self._ind,{BackgroundTransparency=0},"Fast")
end
function Tab:Hide()
	self._ct.Visible=false
	Tw(self._tb,{BackgroundTransparency=1},"Fast")
	Tw(self._nm,{TextColor3=T.TextDim},"Fast")
	Tw(self._ic,{ImageColor3=T.TextMut},"Fast")
	Tw(self._ind,{BackgroundTransparency=1},"Fast")
end
...
-- Footer (bigger, gradient, dividers, executor)
local function MakeFooter(mainF)
	local ft=Instance.new("Frame");ft.Size=UDim2.new(1,0,0,38);ft.Position=UDim2.new(0,0,1,3);ft.BackgroundColor3=T.Surface;ft.BackgroundTransparency=0.05;ft.BorderSizePixel=0;ft.Parent=mainF;Corner(ft,8);Stroke(ft,T.Border,1,0.65)
	Grad(ft,Color3.fromRGB(26,26,36),Color3.fromRGB(20,20,28))

	-- Avatar
	local av=Instance.new("ImageLabel");av.Size=UDim2.fromOffset(26,26);av.Position=UDim2.fromOffset(8,6);av.BackgroundColor3=T.BgAlt;av.BorderSizePixel=0;av.Parent=ft;Corner(av,13)
	...
	-- Username
	local un=Instance.new("TextLabel");un.Size=UDim2.fromOffset(90,26);un.Position=UDim2.fromOffset(38,6);un.BackgroundTransparency=1;un.Text="@"..P.Name;un.TextColor3=T.TextDim;un.Font=T.Font;un.TextSize=10;un.TextXAlignment=Enum.TextXAlignment.Left;un.Parent=ft

	Divider(ft,UDim2.fromOffset(130,10))

	-- Executor icon + name
	local exIc=Instance.new("ImageLabel");exIc.Size=UDim2.fromOffset(14,14);exIc.Position=UDim2.fromOffset(138,12);exIc.BackgroundTransparency=1;exIc.ImageColor3=T.TextMut;exIc.Image=Icon("memory");exIc.Parent=ft
	local exL=Instance.new("TextLabel");exL.Size=UDim2.fromOffset(70,26);exL.Position=UDim2.fromOffset(156,6);exL.BackgroundTransparency=1;exL.Text=DetectExecutor();exL.TextColor3=T.TextMut;exL.Font=T.Font;exL.TextSize=9;exL.TextXAlignment=Enum.TextXAlignment.Left;exL.Parent=ft

	Divider(ft,UDim2.fromOffset(226,10))

	-- FPS
	local fpsIc=Instance.new("ImageLabel");fpsIc.Size=UDim2.fromOffset(12,12);fpsIc.Position=UDim2.fromOffset(234,13);fpsIc.BackgroundTransparency=1;fpsIc.ImageColor3=T.Succ;fpsIc.Image=Icon("speed");fpsIc.Parent=ft
	local fps=Instance.new("TextLabel");fps.Size=UDim2.fromOffset(50,26);fps.Position=UDim2.fromOffset(250,6);fps.BackgroundTransparency=1;fps.Text="--";fps.TextColor3=T.Succ;fps.Font=T.Font;fps.TextSize=9;fps.TextXAlignment=Enum.TextXAlignment.Left;fps.Parent=ft

	Divider(ft,UDim2.fromOffset(300,10))

	-- PING
	local pgIc=Instance.new("ImageLabel");pgIc.Size=UDim2.fromOffset(12,12);pgIc.Position=UDim2.fromOffset(308,13);pgIc.BackgroundTransparency=1;pgIc.ImageColor3=T.Warn;pgIc.Image=Icon("wifi");pgIc.Parent=ft
	local pg=Instance.new("TextLabel");pg.Size=UDim2.fromOffset(60,26);pg.Position=UDim2.fromOffset(324,6);pg.BackgroundTransparency=1;pg.Text="--";pg.TextColor3=T.Warn;pg.Font=T.Font;pg.TextSize=9;pg.TextXAlignment=Enum.TextXAlignment.Left;pg.Parent=ft

	local fc,lt=0,tick()
	RS.Heartbeat:Connect(function()fc=fc+1;local now=tick();if now-lt>=0.5 then fps.Text=math.floor(fc/(now-lt)).." FPS";fc=0;lt=now;pcall(function()pg.Text=math.floor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue()).." ms"end)end end)
end
...
function Win:_build()
	...
	-- Minimize bar (bigger, modern, Material Icons)
	local mbar=Instance.new("Frame");mbar.Size=UDim2.fromOffset(240,38);mbar.Position=UDim2.new(0.5,0,0,8);mbar.AnchorPoint=Vector2.new(0.5,0)
	...
	Grad(mbar,Color3.fromRGB(30,30,42),Color3.fromRGB(22,22,30))
	self._mbar=mbar

	-- Mini logo (square rounded bg)
	local mLogoBg=Instance.new("Frame");mLogoBg.Size=UDim2.fromOffset(28,28);mLogoBg.Position=UDim2.fromOffset(6,5);mLogoBg.BackgroundColor3=T.Accent;mLogoBg.BackgroundTransparency=0.82;mLogoBg.BorderSizePixel=0;mLogoBg.Parent=mbar;Corner(mLogoBg,7)
	local mLogo=Instance.new("ImageLabel");mLogo.Size=UDim2.fromOffset(18,18);mLogo.Position=UDim2.fromOffset(5,5);mLogo.BackgroundTransparency=1;mLogo.Image=self._o.Logo or"";mLogo.Parent=mLogoBg

	-- Mini name (clickable)
	local mBtn=Instance.new("TextButton");mBtn.Size=UDim2.fromOffset(120,38);mBtn.Position=UDim2.fromOffset(38,0);mBtn.BackgroundTransparency=1;mBtn.Text="BloxBox UI";mBtn.TextColor3=T.Text;mBtn.Font=T.FontB;mBtn.TextSize=12;mBtn.TextXAlignment=Enum.TextXAlignment.Left;mBtn.AutoButtonColor=false;mBtn.Parent=mbar
	...
	Divider(mbar,UDim2.fromOffset(162,10))

	-- Drag icon (Material: open_with = move arrows)
	local mDragBg=Instance.new("Frame");mDragBg.Size=UDim2.fromOffset(30,28);mDragBg.Position=UDim2.fromOffset(172,5);mDragBg.BackgroundColor3=T.BgAlt;mDragBg.BackgroundTransparency=0.4;mDragBg.BorderSizePixel=0;mDragBg.Parent=mbar;Corner(mDragBg,6)
	local mDragIc=Instance.new("ImageLabel");mDragIc.Size=UDim2.fromOffset(18,18);mDragIc.Position=UDim2.fromOffset(6,5);mDragIc.BackgroundTransparency=1;mDragIc.ImageColor3=T.TextDim;mDragIc.Image=Icon("open_with");mDragIc.Parent=mDragBg
	mDragBg.InputBegan:Connect(function(i)end) -- make interactable
	...
	Grad(main,Color3.fromRGB(16,16,22),Color3.fromRGB(12,12,16))
	self._mf=main
	...
	Grad(hd,Color3.fromRGB(22,22,30),Color3.fromRGB(18,18,24))
	...
	local clsIc=Instance.new("ImageLabel");clsIc.Size=UDim2.fromOffset(14,14);clsIc.Position=UDim2.fromOffset(5,5);clsIc.BackgroundTransparency=1;clsIc.ImageColor3=T.TextMut;clsIc.Image=Icon("close");clsIc.Parent=cls
	cls.MouseEnter:Connect(function()Tw(cls,{BackgroundTransparency=0.15},"Fast");Tw(clsIc,{ImageColor3=T.Text},"Fast")end)
	...
	-- Minimize (Material icon: remove / horizontal_rule)
	...
	local mnIc=Instance.new("ImageLabel");mnIc.Size=UDim2.fromOffset(14,14);mnIc.Position=UDim2.fromOffset(5,5);mnIc.BackgroundTransparency=1;mnIc.ImageColor3=T.TextMut;mnIc.Image=Icon("remove");mnIc.Parent=mn
	mn.MouseEnter:Connect(function()Tw(mn,{BackgroundTransparency=0.15},"Fast");Tw(mnIc,{ImageColor3=T.Text},"Fast")end)
	...
	-- Sidebar (redesigned with gradient + header label)
	local sb=Instance.new("Frame");sb.Size=UDim2.new(0,130,1,-38);sb.Position=UDim2.fromOffset(0,38);sb.BackgroundColor3=T.BgAlt;sb.BackgroundTransparency=0.2;sb.BorderSizePixel=0;sb.Parent=main
	Grad(sb,Color3.fromRGB(20,20,28),Color3.fromRGB(16,16,22),180)

	-- Sidebar label "NAVIGATION"
	local sbLbl=Instance.new("TextLabel");sbLbl.Size=UDim2.new(1,-10,0,18);sbLbl.Position=UDim2.fromOffset(12,8);sbLbl.BackgroundTransparency=1;sbLbl.Text="NAVIGATION";sbLbl.TextColor3=T.TextMut;sbLbl.Font=T.FontB;sbLbl.TextSize=8;sbLbl.TextXAlignment=Enum.TextXAlignment.Left;sbLbl.Parent=sb

	-- Sidebar divider
	local sbDiv=Instance.new("Frame");sbDiv.Size=UDim2.new(1,-16,0,1);sbDiv.Position=UDim2.fromOffset(8,28);sbDiv.BackgroundColor3=T.Border;sbDiv.BackgroundTransparency=0.45;sbDiv.BorderSizePixel=0;sbDiv.Parent=sb;Corner(sbDiv,1)

	-- Tab list container
	local tlc=Instance.new("Frame");tlc.Size=UDim2.new(1,-10,1,-36);tlc.Position=UDim2.fromOffset(5,34);tlc.BackgroundTransparency=1;tlc.Parent=sb
	local sl=Instance.new("UIListLayout");sl.Padding=UDim.new(0,3);sl.Parent=tlc
	self._tl=tlc

	-- Content separator
	local sep=Instance.new("Frame");sep.Size=UDim2.new(0,1,1,-38);sep.Position=UDim2.fromOffset(130,38);sep.BackgroundColor3=T.Border;sep.BackgroundTransparency=0.35;sep.BorderSizePixel=0;sep.Parent=main

	local cf=Instance.new("Frame");cf.Size=UDim2.new(1,-131,1,-38);cf.Position=UDim2.fromOffset(131,38);cf.BackgroundTransparency=1;cf.BorderSizePixel=0;cf.Parent=main
	self._cf=cf
...
function Win:CreateTab(n)local idx=#self._tabs+1;local tab=Tab.new(self,n,idx);table.insert(self._tabs,tab);if idx==1 then self:SelectTab(tab)end;return tab end
...
print("[BloxBox UI] v2.3 Premium loaded | Material Icons")
return BB
